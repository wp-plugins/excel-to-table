...
class Spreadsheet_Excel_Reader {
	...
	// DUMP AN HTML TABLE OF THE ENTIRE XLS DATA
	// =========================================
	function dump_clean($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$out = "<table class=\"$table_class\" cellspacing=0>";
		$out .= "<tbody>\n";
		...
		//--------------------------
		$endCheckVoidLines = false;
		while (!$endCheckVoidLines){
			for($i=0;$i<$this->colcount($sheet);$i++){
				$tempA[] = $this->val($startRow,$i,$sheet);
			}
			if (count(array_filter($tempA)) == 0) {
				$startRow++;
			}else {
				$endCheckVoidLines= true;
			}
		}
		//--------------------------
		$endCheckVoidLines = false;
		while (!$endCheckVoidLines){
			for($i=0;$i<$this->colcount($sheet);$i++){
				$tempB[] = $this->val($endRow,$i,$sheet);
			}
			
			if (count(array_filter($tempB)) == 0) {
				$endRow--;
			}else {
				$endCheckVoidLines= true;
			}
		...
		$endCheckVoidLines = false;
		while (!$endCheckVoidLines){
			for($j=0;$j<$this->rowcount($sheet);$j++){
				$tempC[] = $this->val($j,$startCol,$sheet);
			}
			if (count(array_filter($tempC)) == 0) {
				$startCol++;
			}else {
				$endCheckVoidLines= true;
			}
		...
		while (!$endCheckVoidLines){	
			for($j=0;$j<$this->rowcount($sheet);$j++){
				$tempD[] = $this->val($j,$endCol,$sheet);
			}
			if (count(array_filter($tempD)) == 0) {
				$endCol--;
			}else {
				$endCheckVoidLines= true;
			}
		...
		$numberOfRow = $this->rowcount($sheet)-1;
		$numberoOfColumn = $this->colcount($sheet)-1;
